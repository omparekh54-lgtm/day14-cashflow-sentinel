...
export default function Page(){
  const [rows,setRows]=useState<Row[]>([]); const [error,setError]=useState(''); const [cash,setCash]=useState(520000); const [scenario,setScenario]=useState(0); const [selected,setSelected]=useState<number|null>(null);
  const today='2026-09-01';
  const fc=useMemo(()=>rows.length?forecast(rows,cash,today,scenario):[],[rows,cash,scenario]);
  const queue=useMemo(()=>rows.length?collectionQueue(rows,today):[],[rows]);
  ...
  return <main>
    ...
    {!rows.length ? <section className="hero card">
      ...
      <section className="toolbar card"><div><span className="eyebrow">MODEL INPUT</span><label>Opening cash<input value={cash} type="number" onChange={e=>setCash(Number(e.target.value)||0)}/></label></div><div><span className="eyebrow">SCENARIO</span><label>Receipts stress: {scenario>0?'+':''}{scenario}%<input type="range" min="-30" max="20" value={scenario} onChange={e=>setScenario(Number(e.target.value))}/></label></div><div className="actions"><label className="btn">Replace CSV<input hidden type="file" accept=".csv" onChange={e=>onFile(e.target.files?.[0])}/></label><button className="btn" onClick={()=>download('collection-queue.csv',exportQueue(queue))}>Export queue</button><button className="link" onClick={()=>setRows([])}>Reset</button></div></section>
